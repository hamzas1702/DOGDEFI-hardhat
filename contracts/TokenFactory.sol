...
contract TokenFactory {
    ...
    function getMemeTokenData(uint index) public view returns (string memory name, string memory symbol, string memory description, string memory imageUrl, address creatorAddress) {
        require(index < memeTokenAddresses.length, "Index out of bounds");
        address tokenAddress = memeTokenAddresses[index];
        memeToken memory token = addressToMemeTokenMapping[tokenAddress];
        return (token.name, token.symbol, token.description, token.tokenImageUrl, token.creatorAddress);
    }
    function getMemeTokenCount() public view returns (uint256) {
